...
object BackupManager : com.example.common.data.util.BackupManager(
    ...
    override fun writeSettings(context: Context): JSONObject {
        ...
        return JSONObject().apply {
            ...
            // Per-folder media custom orders → JSONObject { "bucketId": [videoId, ...] }
            val folderMediaOrdersObj = JSONObject()
            prefs.getAllFolderMediaCustomOrders().forEach { (bucketId, videoIds) ->
                folderMediaOrdersObj.put(bucketId.toString(), JSONArray(videoIds))
            }
