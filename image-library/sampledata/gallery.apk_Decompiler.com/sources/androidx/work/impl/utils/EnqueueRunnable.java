...
public abstract class EnqueueRunnable {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:72:0x0121, code lost:
        r2 = r17.workSpecDao();
        r5 = r2.getWorkSpecIdAndStatesForName(r1).iterator();
     */
        ...
        if (parents != null) {
            for (WorkContinuationImpl next : parents) {
                if (!next.isEnqueued()) {
                    ...
                    String str = TAG;
                    logger.warning(str, "Already enqueued work ids (" + TextUtils.join(ArcCommonLog.TAG_COMMA, next.getIds()) + ")");
                }
            }
        }
