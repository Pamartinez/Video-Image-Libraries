...
public class GreedyScheduler implements Scheduler, OnConstraintsStateChangedListener, ExecutionListener {
    ...
    private final Map<WorkGenerationalId, C0867e0> mConstrainedWorkSpecs = new HashMap();
    ...
    private long throttleIfNeeded(WorkSpec workSpec) {
        ...
        synchronized (this.mLock) {
            try {
                ...
                max = (((long) Math.max((workSpec.runAttemptCount - attemptData.mRunAttemptCount) - 5, 0)) * 30000) + attemptData.mTimeStamp;
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    public void schedule(WorkSpec... workSpecArr) {
        ...
        synchronized (this.mLock) {
            try {
                if (!hashSet.isEmpty()) {
                    String join = TextUtils.join(GlobalPostProcInternalPPInterface.SPLIT_REGEX, hashSet2);
