package U7;

import com.samsung.android.gallery.app.ui.viewer2.remaster.ondemand.RemasterProcessingViewHandler;

/* compiled from: r8-map-id-85efad48717c48307252110572a40cba967e8b6d39747b0efe99f107082547d8 */
public final /* synthetic */ class d implements Runnable {
    public final /* synthetic */ int d;
    public final /* synthetic */ RemasterProcessingViewHandler e;
    public final /* synthetic */ Object[] f;

    public /* synthetic */ d(RemasterProcessingViewHandler remasterProcessingViewHandler, Object[] objArr, int i2) {
        this.d = i2;
        this.e = remasterProcessingViewHandler;
        this.f = objArr;
    }

    public final void run() {
        switch (this.d) {
            case 0:
                this.e.lambda$onRemasterCancel$3(this.f);
                return;
            default:
                this.e.lambda$onRemasterProcessing$4(this.f);
                return;
        }
    }
}
