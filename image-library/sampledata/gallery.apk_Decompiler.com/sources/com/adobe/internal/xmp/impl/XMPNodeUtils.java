...
public class XMPNodeUtils implements XMPConst {
    ...
    public static void appendLangItem(XMPNode xMPNode, String str, String str2) {
    ...
    public static Object[] chooseLocalizedText(XMPNode xMPNode, String str, String str2) {
    ...
    public static XMPNode findChildNode(XMPNode xMPNode, String str, boolean z) {
    ...
    public static XMPNode findNode(XMPNode xMPNode, XMPPath xMPPath, boolean z, PropertyOptions propertyOptions) {
        XMPNode xMPNode2;
        ...
        int i2 = 1;
        while (i2 < xMPPath.size()) {
            try {
                findSchemaNode = followXPathStep(findSchemaNode, xMPPath.getSegment(i2), z);
                if (findSchemaNode == null) {
                    if (z) {
                        deleteNode(xMPNode2);
                    }
                ...
                if (findSchemaNode.isImplicit()) {
                    ...
                    if (i2 == 1 && xMPPath.getSegment(i2).isAlias() && xMPPath.getSegment(i2).getAliasForm() != 0) {
                        findSchemaNode.getOptions().setOption(xMPPath.getSegment(i2).getAliasForm(), true);
                    } else if (i2 < xMPPath.size() - 1 && xMPPath.getSegment(i2).getKind() == 1 && !findSchemaNode.getOptions().isCompositeProperty()) {
    ...
    public static XMPNode findSchemaNode(XMPNode xMPNode, String str, boolean z) {
    ...
    private static XMPNode followXPathStep(XMPNode xMPNode, XMPPathSegment xMPPathSegment, boolean z) {
        ...
        if (xMPNode.getOptions().isArray()) {
            if (kind == 3) {
                i2 = findIndexedItem(xMPNode, xMPPathSegment.getName(), z);
            } else if (kind == 4) {
                i2 = xMPNode.getChildrenLength();
            } else if (kind == 6) {
                String[] splitNameAndValue = Utils.splitNameAndValue(xMPPathSegment.getName());
                i2 = lookupFieldSelector(xMPNode, splitNameAndValue[0], splitNameAndValue[1]);
            } else if (kind == 5) {
                String[] splitNameAndValue2 = Utils.splitNameAndValue(xMPPathSegment.getName());
                i2 = lookupQualSelector(xMPNode, splitNameAndValue2[0], splitNameAndValue2[1], xMPPathSegment.getAliasForm());
            } else {
                throw new XMPException("Unknown array indexing step in FollowXPathStep", 9);
            }
            if (1 > i2 || i2 > xMPNode.getChildrenLength()) {
                return null;
            }
            return xMPNode.getChild(i2);
        }
    ...
    private static int lookupFieldSelector(XMPNode xMPNode, String str, String str2) {
        int i2 = -1;
        int i7 = 1;
        while (i7 <= xMPNode.getChildrenLength() && i2 < 0) {
            XMPNode child = xMPNode.getChild(i7);
            if (child.getOptions().isStruct()) {
                int i8 = 1;
                while (true) {
                    if (i8 > child.getChildrenLength()) {
                        break;
                    }
                    XMPNode child2 = child.getChild(i8);
    ...
    public static int lookupLanguageItem(XMPNode xMPNode, String str) {
        if (xMPNode.getOptions().isArray()) {
    ...
    private static int lookupQualSelector(XMPNode xMPNode, String str, String str2, int i2) {
        ...
        for (int i7 = 1; i7 < xMPNode.getChildrenLength(); i7++) {
            Iterator iterateQualifier = xMPNode.getChild(i7).iterateQualifier();
    ...
    public static XMPNode findSchemaNode(XMPNode xMPNode, String str, String str2, boolean z) {
        XMPNode findChildByName = xMPNode.findChildByName(str);
        if (findChildByName != null || !z) {
            return findChildByName;
        }
