...
public class XMPNormalizer {
    ...
    private static void migrateAudioCopyright(XMPMeta xMPMeta, XMPNode xMPNode) {
        try {
            ...
            xMPMeta2.setLocalizedText(XMPConst.NS_DC, "rights", "", XMPConst.X_DEFAULT, "\n\n" + value, (PropertyOptions) null);
            xMPNode.getParent().removeChild(xMPNode);
        } catch (XMPException unused) {
        }
    }
    ...
    private static void touchUpDataModel(XMPMetaImpl xMPMetaImpl) {
        ...
        while (iterateChildren.hasNext()) {
            ...
            if (XMPConst.NS_DC.equals(xMPNode.getName())) {
                ...
                if (findChildNode2 != null) {
                    if (findChildNode2.getOptions().isSimple()) {
                        XMPNode xMPNode2 = new XMPNode(XMPConst.ARRAY_ITEM_NAME, findChildNode2.getValue(), findChildNode2.getOptions());
                        xMPNode2.setParent(findChildNode2);
                        for (int qualifierLength = findChildNode2.getQualifierLength(); qualifierLength > 0; qualifierLength--) {
                            xMPNode2.addQualifier(findChildNode2.getQualifier(findChildNode2.getQualifierLength() - qualifierLength));
                        }
                        findChildNode2.removeQualifiers();
                        ...
                        findChildNode2.addChild(xMPNode2);
                        findChildNode2.setOptions(new PropertyOptions(7680));
                        findChildNode2.setValue("");
                    }
        ...
        if (xMPNode.getName() != null && xMPNode.getName().length() >= 36) {
            ...
            if (Utils.checkUUIDFormat(lowerCase)) {
                ...
                if (findNode != null) {
                    ...
                    findNode.setValue(C0212a.l("uuid:", lowerCase));
