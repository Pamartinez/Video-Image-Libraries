...
public class TypeToken<T> {
            ...
            if (length2 != length) {
                ...
                if (!GsonTypes.requiresOwnerType(type2)) {
                    for (int i2 = 0; i2 < length; i2++) {
                        ...
                        int i7 = 0;
                        while (i7 < length3) {
                            if (GsonTypes.getRawType(bounds[i7]).isAssignableFrom(rawType2)) {
                                i7++;
                            } else {
    ...
    private static boolean isCapturingTypeVariablesForbidden() {
        return !Objects.equals(System.getProperty("gson.allowCapturingTypeVariables"), SpenRecogConfig.OCR_RECOGNIZER_CONFIGURATION_VAL_TRUE);
    }
    ...
    public boolean isAssignableFrom(Type type2) {
        ...
        if (type3 instanceof GenericArrayType) {
            return this.rawType.isAssignableFrom(GsonTypes.getRawType(type2)) && isAssignableFrom(type2, (GenericArrayType) this.type);
        }
