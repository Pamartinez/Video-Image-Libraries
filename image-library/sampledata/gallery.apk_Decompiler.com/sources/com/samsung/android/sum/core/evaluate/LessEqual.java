...
public class LessEqual<T extends Comparable<T>> extends GenericEvaluator<T> {
    public static final Parcelable.Creator<LessEqual<?>> CREATOR = new Parcelable.Creator<LessEqual<?>>() {
        public LessEqual<?> createFromParcel(Parcel parcel) {
            return new LessEqual<>(parcel);
        }

        public LessEqual<?>[] newArray(int i2) {
            return new LessEqual[i2];
        }
    };

    public LessEqual(T t) {
        super(t);
    }
    ...
    public LessEqual(Parcel parcel) {
        super(parcel);
    }
}
