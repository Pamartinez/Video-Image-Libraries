...
public class VSWUpscale2xEnhancer implements IEnhancer {
    private static final String TAG = "Remaster-VSWUpscale2xEnhancer";
    ...
    public List<String> getEstimators(Context context) {
        return List.of(VSWUpscale2xEnhancer.class.getName());
    }
    ...
    public EnhanceResult enhance(Context context, BgrBuffer bgrBuffer, BgrBuffer bgrBuffer2, double d, List<Rect> list, String str, IRequestCancelledChecker iRequestCancelledChecker) {
        ...
        if (str != null && str.length() > 0) {
            try {
                jSONObject = new JSONObject(str).getJSONObject(VSWUpscale2xEnhancer.class.getName()).getJSONObject("estimatedDataJson");
