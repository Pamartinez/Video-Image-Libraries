...
public class VSWClarity1xEnhancer implements IEnhancer {
    private static final String TAG = "Remaster-VSWClarity1xEnhancer";
    ...
    public PrSize getOutputSizeFrom(Context context, PrSize prSize) {
        return new PrSize(prSize.getWidth(), prSize.getHeight());
    }
    ...
    public EnhanceResult enhance(Context context, BgrBuffer bgrBuffer, BgrBuffer bgrBuffer2, double d, List<Rect> list, String str, IRequestCancelledChecker iRequestCancelledChecker) {
        ...
        if (str != null && str.length() > 0) {
            try {
                jSONObject = new JSONObject(str).getJSONObject(VSWClarity1xEnhancer.class.getName()).getJSONObject("estimatedDataJson");
