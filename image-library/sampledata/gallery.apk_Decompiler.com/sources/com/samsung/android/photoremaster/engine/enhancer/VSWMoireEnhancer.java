...
public class VSWMoireEnhancer implements IEnhancer {
    private static final String TAG = "Remaster-VSWMoireEnhancer";

    public static boolean enhance(byte[] bArr, byte[] bArr2, int i2, int i7) {
        String str = TAG;
        Log.d(str, "enhance - E");
        VSWEngineNativeInterface.printLibraryVersion();
        C0280e.g("result : ", str, str, "enhance - X", VSWEngineNativeInterface.runMoireEnhancer(bArr, bArr2, i2, i7));
        return true;
    }

    public List<String> getEstimators(Context context) {
        return List.of(VSWMoireEnhancer.class.getName());
    }
    ...
    public EnhanceResult enhance(Context context, BgrBuffer bgrBuffer, BgrBuffer bgrBuffer2, double d, List<Rect> list, String str, IRequestCancelledChecker iRequestCancelledChecker) {
        ...
        if (str != null && str.length() > 0) {
            try {
                jSONObject = new JSONObject(str).getJSONObject(VSWMoireEnhancer.class.getName()).getJSONObject("estimatedDataJson");
        ...
        if (iRequestCancelledChecker == null || !iRequestCancelledChecker.isRequestCancelled()) {
            return new EnhanceResult(enhance(data, data2, size.getWidth(), size.getHeight()));
        }
