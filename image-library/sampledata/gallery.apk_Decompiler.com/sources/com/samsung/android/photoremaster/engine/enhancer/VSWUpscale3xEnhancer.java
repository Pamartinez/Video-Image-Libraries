...
public class VSWUpscale3xEnhancer implements IEnhancer {
    private static final String TAG = "Remaster-VSWUpscale3xEnhancer";

    public static boolean enhance(byte[] bArr, byte[] bArr2, int i2, int i7, int i8) {
        String str = TAG;
        Log.d(str, "enhance - E");
        VSWEngineNativeInterface.printLibraryVersion();
        C0280e.g("result : ", str, str, "enhance - X", VSWEngineNativeInterface.runResolutionEnhancer(bArr, bArr2, i2, i7, i8));
        return true;
    }

    public List<String> getEstimators(Context context) {
        return List.of(VSWUpscale3xEnhancer.class.getName());
    }

    public PrSize getOutputSizeFrom(Context context, PrSize prSize) {
        return new PrSize(prSize.getWidth() * 3, prSize.getHeight() * 3);
    }
    ...
    public EnhanceResult enhance(Context context, BgrBuffer bgrBuffer, BgrBuffer bgrBuffer2, double d, List<Rect> list, String str, IRequestCancelledChecker iRequestCancelledChecker) {
        ...
        if (str != null && str.length() > 0) {
            try {
                jSONObject = new JSONObject(str).getJSONObject(VSWUpscale3xEnhancer.class.getName()).getJSONObject("estimatedDataJson");
