...
public class VSWUpscale4xEnhancer implements IEnhancer {
    private static final String TAG = "Remaster-VSWUpscale4xEnhancer";
    ...
    public List<String> getEstimators(Context context) {
        return List.of(VSWUpscale4xEnhancer.class.getName());
    }

    public PrSize getOutputSizeFrom(Context context, PrSize prSize) {
        return new PrSize(prSize.getWidth() * 4, prSize.getHeight() * 4);
    }
    ...
    public EnhanceResult enhance(Context context, BgrBuffer bgrBuffer, BgrBuffer bgrBuffer2, double d, List<Rect> list, String str, IRequestCancelledChecker iRequestCancelledChecker) {
        ...
        if (str != null && str.length() > 0) {
            try {
                jSONObject = new JSONObject(str).getJSONObject(VSWUpscale4xEnhancer.class.getName()).getJSONObject("estimatedDataJson");
