...
public class GenericInferenceInferenceRunnable extends TaskRunnable<GenericInferenceResult> {
    private static final String TAG = "GenericInferenceInferenceRunnable";
    private String mFeature;
    private int mMode;
    private final Bundle mRequestBundle = new Bundle();
    private final ExtensionServiceExecutor mServiceExecutor;

    public GenericInferenceInferenceRunnable(ExtensionServiceExecutor extensionServiceExecutor) {
    ...
    private void setResult(Bundle bundle) {
        ...
        bundle.setClassLoader(TaskRunnable.class.getClassLoader());
        GenericInferenceResult genericInferenceResult = new GenericInferenceResult();
        int i2 = bundle.getInt(OCRServiceConstant.KEY_RESULT_CODE);
        genericInferenceResult.setResult(i2);
        if (i2 != 1) {
            ...
            genericInferenceResult.setBundle(bundle);
            this.mSource.setResult(genericInferenceResult);
        }
    }

    public void execute() {
        Log.d(TAG, "GenericInferenceInferenceRunnable execute()");
        if (getBundle() == null || ExtensionParamUtils.getGenericInferenceMethod(getBundle()) == null) {
    ...
    public String getFeatureName() {
        return Feature.FEATURE_CORE_EXT_GENERIC_INFERENCE;
    }
