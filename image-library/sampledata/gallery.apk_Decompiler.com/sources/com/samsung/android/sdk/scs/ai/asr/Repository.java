...
interface Repository {
    ...
    public static class SharedPrefRepository implements Repository {
        ...
        public static /* synthetic */ ServerType lambda$getServerType$2(String str, ServerFeature serverFeature, SharedPreferences sharedPreferences) {
            return (ServerType) Optional.ofNullable(sharedPreferences.getString(str, (String) null)).map(new d(serverFeature, sharedPreferences, str, 1)).orElse((Object) null);
        }
        ...
        public ServerType getServerType(ServerFeature serverFeature) {
            return (ServerType) Optional.ofNullable(this.sharedPrefSupplier).map(new i(getKey("server_type", serverFeature), serverFeature)).orElse((Object) null);
        }
        ...
        public boolean setServerType(ServerFeature serverFeature, ServerType serverType) {
            ...
            try {
                return ((Boolean) Optional.ofNullable(this.sharedPrefSupplier).map(new g(1)).map(new i(serverType, getKey("server_type", serverFeature))).map(new g(2)).orElse(Boolean.FALSE)).booleanValue();
            } catch (Exception e7) {
                String str2 = this.TAG;
                Log.w(str2, "Failed to set server type " + e7.getMessage());
                return false;
            }
        }
