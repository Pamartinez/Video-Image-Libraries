package com.samsung.android.imagetranslation.util;

import com.samsung.android.imagetranslation.data.LttOcrResult;

/* compiled from: r8-map-id-85efad48717c48307252110572a40cba967e8b6d39747b0efe99f107082547d8 */
class BlockInfoUtil {
    /* JADX WARNING: type inference failed for: r0v1, types: [java.lang.Object, java.util.Comparator] */
    public static int getMaxLineWidth(LttOcrResult.BlockInfo blockInfo) {
        return blockInfo.getLineInfo().stream().map(new a(1)).max(new Object()).orElse(0).intValue();
    }

    /* JADX WARNING: type inference failed for: r1v1, types: [java.lang.Object, java.util.function.BinaryOperator] */
    public static int getWordCount(LttOcrResult.BlockInfo blockInfo) {
        return blockInfo.getLineInfo().stream().map(new a(0)).reduce(0, new Object()).intValue();
    }
}
