...
class LineWidthRule implements Rule {
    ...
    public static class LineOutlierDetector {
        ...
        private static double getStdDeviation(LttOcrResult.BlockInfo blockInfo, double d) {
            ...
            return Math.sqrt((blockInfo.getLineInfo().stream().mapToDouble(new Object()).reduce(MapUtil.INVALID_LOCATION, new i(d)) / ((double) blockInfo.getLineInfo().size())) - 1.0d);
        }
    ...
    public /* synthetic */ void lambda$getScore$4(AnonymousClass1 r9, LttOcrResult.BlockInfo blockInfo) {
        ...
        double count = (double) blockInfo.getLineInfo().stream().filter(new d(0, lineOutlierDetector)).peek(new b(1, r32)).filter(new e(maxLineWidth)).count();
        double d = r9.score;
        double d2 = r32.value;
    ...
    public double getScore(List<LttOcrResult.BlockInfo> list) {
        ...
        list.stream().filter(new f(0)).forEach(new g(this, r0));
