    ...
    private static void validatePadding(CopyOnWriteArrayList<SceneText> copyOnWriteArrayList, int i2, int i7) {
        ...
        for (int i8 = 0; i8 < size; i8++) {
            ...
            for (int i10 = 0; i10 < size; i10++) {
                if (i8 != i10 || size == 1) {
                    if (LiveTranslation.NATIVE_API_VERSION >= 2) {
                        pointArr2 = copyOnWriteArrayList.get(i8).getPaddedBoxPoly();
                        pointArr = copyOnWriteArrayList.get(i10).getPaddedBoxPoly();
                    } else {
                        pointArr2 = copyOnWriteArrayList.get(i8).getBoxPoly();
                        pointArr = copyOnWriteArrayList.get(i10).getBoxPoly();
                    }
                    while (dynamicPadding > 0 && ((size != 1 && isRectIntersecting(pointArr2, pointArr)) || isPolyOutOfBound(pointArr2, i2, i7))) {
                        dynamicPadding--;
                        updatePoly(copyOnWriteArrayList.get(i8), dynamicPadding, copyOnWriteArrayList.get(i8).getPoly());
                        if (LiveTranslation.NATIVE_API_VERSION >= 2) {
                            boxPoly = copyOnWriteArrayList.get(i8).getPaddedBoxPoly();
                        } else {
                            boxPoly = copyOnWriteArrayList.get(i8).getBoxPoly();
                        }
                    }
                }
            }
