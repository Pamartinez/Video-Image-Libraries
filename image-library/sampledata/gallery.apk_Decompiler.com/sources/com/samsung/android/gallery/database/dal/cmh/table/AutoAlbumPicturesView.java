...
public class AutoAlbumPicturesView extends BaseView {
    ...
    public void filterAlbumId(int i2) {
        ...
        queryBuilder.andCondition("AU.album_id = " + i2);
    }
    ...
    public String getSortByForDateCursor(int i2) {
        ...
        return this.mFilesTable.getAlbumOrderByQuery(i2).replace("A.date_modified", "__dateModified").replace("A.datetaken", "__dateTaken").replace(", A._ID DESC", "").replace(", A._ID ASC", "");
    }
    ...
    public void filterAlbumId(Collection<Integer> collection) {
        ...
        QueryBuilder queryBuilder = this.mQueryBuilder;
        queryBuilder.andCondition("AU.album_id in (" + TextUtils.join(GlobalPostProcInternalPPInterface.SPLIT_REGEX, collection) + ")");
    }
}
