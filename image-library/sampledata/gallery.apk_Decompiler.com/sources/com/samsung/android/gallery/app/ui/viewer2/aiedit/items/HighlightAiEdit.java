...
public class HighlightAiEdit extends AiEditItem {
    ...
    public HighlightAiEdit(EventContext eventContext, ActionInvoker actionInvoker) {
        super(eventContext, actionInvoker, AiEditType.Highlights);
    }

    public void execute(MediaItem mediaItem, boolean z) {
        super.lambda$executeItem$0(mediaItem, z);
        if (mediaItem != null) {
            new VuLauncher(this.mEventContext.getBlackboard()).publishData().setIsTemp().disableTimeline().prepareThumbnail().forcePlayVideoInGallery(PreferenceFeatures.VideoPlayerFeature.SUPPORT_SETTING).launch("location://highlightViewList", 0, false, this.mBestItem);
        } else {
    ...
    public List<AiEditItem> load(MediaItem mediaItem, Bitmap bitmap) {
        MediaItem highlightBestItem = DynamicViewUtils.getHighlightBestItem(mediaItem);
        this.mBestItem = highlightBestItem;
        if (highlightBestItem != null) {
            return List.of(this);
        }
        return null;
    }
