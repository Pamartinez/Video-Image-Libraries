...
public class SuggestedCleanOutView extends DbTable {
    ...
    private String getFileIdColumnName() {
        ...
        return "files.sec_media_id";
    }

    private String getFilterDeleteGroupID() {
        StringBuilder sb2 = new StringBuilder("delete_group_id in (select delete_group_id from delete_recommendation INNER JOIN files ON (delete_recommendation.fk_file_id = files._id AND (is_favorite is null OR is_favorite = 0) AND isUsedAsWallpaper = 0) where ");
        sb2.append(getWhere());
    ...
    public void resetProjectionForCover() {
        ...
        if (!SdkConfig.atLeast(SdkConfig.GED.Q)) {
            QueryBuilder queryBuilder = this.mQueryBuilder;
            queryBuilder.addProjection("(select count(*) from " + getTableNameRaw() + " INNER JOIN files ON (delete_recommendation.fk_file_id = files._id AND (is_favorite is null OR is_favorite = 0) AND isUsedAsWallpaper = 0) AND " + getFilterHidden() + ")", KeywordInfoBundleWrapper.BUNDLE_KEY_EXTRA);
        } else if (this.USE_GMP) {
            String mountedVolumeNames = FileUtils.getMountedVolumeNames();
            QueryBuilder queryBuilder2 = this.mQueryBuilder;
            queryBuilder2.addProjection("(select count(*) from " + getTableNameRaw() + " INNER JOIN files ON (delete_recommendation.fk_file_id = files._id AND (is_favorite is null OR is_favorite = 0) AND isUsedAsWallpaper = 0) AND files.volume_name in (" + mountedVolumeNames + ") AND " + getFilterHidden() + ")", KeywordInfoBundleWrapper.BUNDLE_KEY_EXTRA);
        } else {
            QueryBuilder queryBuilder3 = this.mQueryBuilder;
            queryBuilder3.addProjection("(select count(*) from " + getTableNameRaw() + " INNER JOIN files ON (delete_recommendation.fk_file_id = files._id AND (is_favorite is null OR is_favorite = 0) AND isUsedAsWallpaper = 0) AND files.is_mount = 1 AND " + getFilterHidden() + ")", KeywordInfoBundleWrapper.BUNDLE_KEY_EXTRA);
        }
    }
    ...
    public void resetProjectionForCoverV2(int i2) {
        ...
        if (!SdkConfig.atLeast(SdkConfig.GED.Q)) {
            QueryBuilder queryBuilder = this.mQueryBuilder;
            queryBuilder.addProjection("(select count(*) from " + getTableNameRaw() + " INNER JOIN files ON (delete_recommendation.fk_file_id = files._id AND (is_favorite is null OR is_favorite = 0) AND isUsedAsWallpaper = 0) AND delete_type = " + i2 + " AND " + getFilterHidden() + ")", KeywordInfoBundleWrapper.BUNDLE_KEY_EXTRA);
        } else if (this.USE_GMP) {
            String mountedVolumeNames = FileUtils.getMountedVolumeNames();
            QueryBuilder queryBuilder2 = this.mQueryBuilder;
            queryBuilder2.addProjection("(select count(*) from " + getTableNameRaw() + " INNER JOIN files ON (delete_recommendation.fk_file_id = files._id AND (is_favorite is null OR is_favorite = 0) AND isUsedAsWallpaper = 0) AND clean_state = 0 AND files.volume_name in (" + mountedVolumeNames + ") AND delete_type = " + i2 + " AND " + getFilterHidden() + ")", KeywordInfoBundleWrapper.BUNDLE_KEY_EXTRA);
        } else {
            QueryBuilder queryBuilder3 = this.mQueryBuilder;
            queryBuilder3.addProjection("(select count(*) from " + getTableNameRaw() + " INNER JOIN files ON (delete_recommendation.fk_file_id = files._id AND (is_favorite is null OR is_favorite = 0) AND isUsedAsWallpaper = 0) AND clean_state = 0 AND files.is_mount = 1 AND delete_type = " + i2 + " AND " + getFilterHidden() + ")", KeywordInfoBundleWrapper.BUNDLE_KEY_EXTRA);
        }
    }
    ...
    public void resetProjectionForDuplicateCover() {
        resetDefaultProjectionForCover();
        QueryBuilder queryBuilder = this.mQueryBuilder;
        queryBuilder.addProjection("(select count(distinct " + getFileIdColumnName() + ") from " + getTableNameRaw() + " INNER JOIN files ON (delete_recommendation.fk_file_id = files._id AND (is_favorite is null OR is_favorite = 0) AND isUsedAsWallpaper = 0) AND delete_type = 3 AND " + getFilterDeleteGroupID() + ")", KeywordInfoBundleWrapper.BUNDLE_KEY_EXTRA);
    }
    ...
    public void setDefaultCondition() {
        ...
        if (!SdkConfig.atLeast(SdkConfig.GED.Q)) {
            ...
            str = "clean_state = 0 AND files.is_mount = 1";
        }
